...
//변수에 저장되어 있는 데이터의 값은 일반적으로 언제든지 변경이 가능하다
// c언어에서의 변수는 변수마다 정해진 데이터의 형태(자료형)가 존재하며, 각변수는
//할당된 값을 가지고 있습니다.
...
//float   실수       4byte   3.4 * 10 -38제곱 ~ 3.4*10 +38제곱 -->소수점 이하6자리까지 정확하게 표현
...
int main()
{
	...
	// 변수의 선언과 초기화를 동시에 진핼 할 수 있다
	float PI = 3.141592f;

	//데이터의 형태가 같다면, 여러 개를 한번에 선언할 수 있다
	int a, b, c;

	//선언과 초기화를 동시에 여러 개를 진행할 수 있습니다
	int number3 = 50, number4 = 40;

	//c언어에서 10진수 이외의 8진수나 16진수를 지정할 수 있다
	int octo = 015; //tntwk dkvdp 0이 붙으면 8진수
